...
actor ConversationParser {
    ...
    /// Build session file path from session ID and working directory.
    /// Only providers that write JSONL files (Claude Code CLI) will have
    /// matching files. Other providers (Cursor, etc.) rely solely on
    /// real-time hook events for their session state.
